...
const get = async (req: Request, res: Response) => {
    try {
        ...
        const [items, totalCount] = await Promise.all([
            Services.findAll(queryOptions),
            Services.count({ where }),
        ]);
...
const destroy = async (req: Request, res: Response) => {
    try {
        ...
        const numDeleted = await Services.destroy({
            // @ts-ignore
            where: {
                id: id,
            },
...
const getData = async (id: any) => {
    try {
        const itemData = await Services.findByPk(id, {
            attributes: project,
        });
...
const store = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [itemData, created] = await Services.upsert(bodyData, {

...
const destroy = async (req: Request, res: Response) => {
    try {
        ...
        const numDeleted = await Team.destroy({
            // @ts-ignore
            where: {
                id: id,
            },
...
const getData = async (id: any) => {
    try {
        const itemData = await Team.findByPk(id, {
            attributes: project,
        });
...
const store = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [itemData, created] = await Team.upsert(bodyData, {

...
const titleName = "Slider";
const allFiled = ["id", "title", "image_path", "is_active", "createdAt"];
const project: any = {};
...
const destroy = async (req: Request, res: Response) => {
  try {
    ...
    const numDeleted = await Slider.destroy({
      // @ts-ignore
      where: {
        id: id,
      },
...
const getData = async (id: any) => {
  try {
    const itemData = await Slider.findByPk(id, {
      attributes: project,
    });
...
const store = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [itemData, created] = await Slider.upsert(bodyData, {
